...
#/home/"$USER"/little-backup-box.log
 #tail -n 7  little-backup-box.log |grep %|tail -n 1|awk '{print $3}'
...
#       Speed: 17.11MB/s
#       53.7'C CPU:0.15
          $copyPercentage=$(tail -n 7  little-backup-box.log |grep %|tail -n 1|awk '{print $3}')
        $messageA="Copying... $copyPercentage"
          $copySize=$(tail -n 7 little-backup-box.log |grep %|tail -n 1|awk '{print $2}')
          $ElapseTime=$(tail -n 7 little-backup-box.log |grep %|tail -n 1|awk '{print $5}')
        $messageB="$copySize $ElapseTime"
          $copySpeed=$(tail -n 7 little-backup-box.log |grep %|tail -n 1|awk '{print $4}')
